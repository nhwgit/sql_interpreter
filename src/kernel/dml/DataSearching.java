...
public class DataSearching {
	...
	private String whereStatement = null; // 조인 구현 완료
	private String groupByStatement = null; //ver2에서 구현 예정
	private String havingStatement = null; //ver2에서 구현 예정
	...
	public void queryParsing() {
		...
		Pattern fromPattern = Pattern.compile("FROM\\s+(.*?)((\\s+(WHERE|ORDER BY))|$)");
		...
		if(matcher.find()) fromStatement = matcher.group(1).trim();
		else throw new InvalidSyntaxException();
